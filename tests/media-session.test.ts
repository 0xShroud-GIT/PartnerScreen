...
test('production media stats are sanitized, measured, and can mark publishing degraded', async () => {
  ...
  h.native.stats = { bytesSent: 10_000, packetsLost: 0, framesPerSecond: 20, bitrateParametersState: 'applied' };
  ...
  now = 2_500;
  h.native.stats = { bytesSent: 80_000, packetsLost: 24, roundTripTime: 0.45, bitrateParametersState: 'failed' };
  h.scheduler.runNext(); await settle();
  ...
  assert.equal(h.media.getStatsSnapshot()?.bitrateParametersState, 'failed');
