...
test('app-level keyframe timing policy is gone', () => {
  const policy = require('../src/media/MediaPolicy') as Record<string, unknown>;
  assert.equal('MEDIA_KEYFRAME_REQUEST_DELAYS_MS' in policy, false);
  assert.equal('MEDIA_KEYFRAME_STEADY_RETRY_MS' in policy, false);
  assert.equal('MEDIA_KEYFRAME_TOGGLE_MS' in policy, false);
  assert.equal('keyframeRetryDelayMs' in policy, false);
});
...
test('sender bitrate policy returns a safe patch for a real encoding', () => {
...
test('ICE policy covers all three RFC1918 address blocks', () => {
  ...
  assert.equal(classifyIceCandidate('candidate:1 1 UDP 2 172.32.0.1 50000 typ host').accepted, false);
  assert.equal(classifyIceCandidate('candidate:1 1 UDP 2 192.168.255.255 50000 typ host').accepted, true);
  assert.equal(classifyIceCandidate('candidate:1 1 UDP 2 192.169.0.1 50000 typ host').accepted, false);
});
