...
for (const marker of ['ServerSocket()', 'activeWifiEndpoint', 'connectivity.activeNetwork ?: return null', 'wifi.network.bindSocket(socket)', 'socket.bind(InetSocketAddress(wifi.address, 0))', 'socket.connect(InetSocketAddress(address, port), CONNECT_TIMEOUT_MS)', 'MAX_FRAME_BYTES', 'readInt()', 'writeInt(bytes.size)', 'shutdownAll()']) requireMarker(nativeControl, marker, 'PartnerControlModule.kt');
for (const marker of ['connectivity.allNetworks', 'NetworkInterface', 'getNetworkInterfaces', 'Log.', 'println(', 'pairSecret', 'partnerDeviceId', 'partnerDeviceName', 'SharedPreferences', 'AsyncStorage']) forbidMarker(nativeControl, marker, 'PartnerControlModule.kt');
