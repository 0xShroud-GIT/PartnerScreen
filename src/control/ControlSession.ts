...
const RECONNECT_DELAYS_MS = [350, 900, 1_800, 3_500, 5_000] as const;
const RESPONDER_RECONNECT_WINDOW_MS = 18_000;
...
export class ControlSession {
  ...
  updateReconnectEndpoint(endpoint: { host: string; port: number }): void { this.outboundEndpoint = endpoint; }
  ...
  async ensureListening(expectedHost?: string): Promise<{ host: string; port: number }> {
    return this.enqueueResult(async () => { const endpoint = await this.ensureListeningNow(expectedHost); return { host: endpoint.host, port: endpoint.port }; });
  }
  ...
  async connect(endpoint: { host: string; port: number }): Promise<string> {
    ...
    return this.openInitiator(endpoint, sessionId, context, true);
  }
  ...
  async send<T extends ControlMessageType>(type: T, payload: ControlPayloadMap[T]): Promise<AnyControlMessage> {
    return this.enqueueResult(async () => {
      ...
      const message = {
        version: CONTROL_PROTOCOL_VERSION, messageId: this.crypto.randomId(), type, sessionId: active.sessionId,
        senderDeviceId: context.localDeviceId, sequence: active.nextSendSequence++, timestamp: new Date(this.nowMs()).toISOString(), payload,
      } as AnyControlMessage;
      const sealed = await this.crypto.sealMessage(active.sessionKeyHex, message);
      try { await this.transport.send(active.connectionId, encodeSealedControlFrame(sealed)); }
      catch (error) { await this.beginReconnect(active); throw error; }
      return message;
    });
  }
  ...
  private async openInitiator(endpoint: { host: string; port: number }, sessionId: string, context: ControlTrustContext, awaitAuthentication: boolean): Promise<string> {
    ...
    const active: ActiveConnection = {
      connectionId, role: 'initiator', sessionId, initiatorNonce, responderNonce: null, sessionKeyHex: null,
      validator: null, nextSendSequence: 1, handshakeTimer: null, authResolve: resolveAuth, authReject: rejectAuth,
    };
    this.active = active;
    ...
    const unsigned: Omit<Hello1Frame, 'mac'> = {
      ...
      senderDeviceId: context.localDeviceId, nonce: initiatorNonce, timestamp: new Date(this.nowMs()).toISOString(),
    };
    const hello: Hello1Frame = { ...unsigned, mac: await this.crypto.hello1Mac(context.pairSecretHex, unsigned) };
    try { await this.transport.send(connectionId, encodeHandshakeFrame(hello)); }
    catch (error) {
      active.authReject?.(error instanceof Error ? error : new Error('Control handshake failed.'));
  ...
  private async handleTransportEvent(event: ControlTransportEvent): Promise<void> {
    ...
    if (event.type === 'connected') {
      ...
      if (!this.context || this.active || this.resumeRole === 'initiator') { await this.transport.close(event.connectionId).catch(() => undefined); return; }
      const active: ActiveConnection = {
        connectionId: event.connectionId, role: 'responder', sessionId: this.resumeSessionId, initiatorNonce: null,
        responderNonce: null, sessionKeyHex: null, validator: null, nextSendSequence: 1, handshakeTimer: null, authResolve: null, authReject: null,
      };
      this.active = active; this.armHandshakeTimeout(active); return;
    }
    ...
    if (event.type === 'closed') {
      ...
      if (active.sessionKeyHex && active.sessionId) await this.beginReconnect(active);
        ...
        this.clearHandshake(active); active.authReject?.(new Error('Control connection closed before authentication.')); this.active = null;
    ...
    if (event.connectionId) {
      ...
      if (event.code === 'busy' && !active.sessionKeyHex) {
        active.authReject?.(new Error('Partner is busy.')); await this.closeActiveNow(false); this.emit({ type: 'error', code: 'busy' });
      } else if (active.sessionKeyHex && active.sessionId) await this.beginReconnect(active);
      else if (this.resumeSessionId) { await this.transport.close(active.connectionId).catch(() => undefined); this.active = null; this.scheduleInitiatorReconnect(); }
      else { this.emit({ type: 'error', code: 'transport_failed' }); await this.closeActiveNow(false); }
      return;
    }
  ...
  private async beginReconnect(active: ActiveConnection): Promise<void> {
    ...
    const sessionId = active.sessionId; const role = active.role;
    this.clearHandshake(active); this.active = null;
    active.authReject?.(new Error('Control transport interrupted.'));
    await this.transport.close(active.connectionId).catch(() => undefined);
    this.resumeSessionId = sessionId; this.resumeRole = role; this.reconnectAttempt = 0;
    this.emit({ type: 'reconnecting', sessionId, role, attempt: 1 });
    if (role === 'initiator') this.scheduleInitiatorReconnect(); else this.armResponderReconnectWindow();
  }

  private scheduleInitiatorReconnect(): void {
    if (!this.resumeSessionId || this.resumeRole !== 'initiator' || this.reconnectTimer) return;
    if (this.reconnectAttempt >= RECONNECT_DELAYS_MS.length) { void this.enqueue(() => this.failReconnect()).catch(() => undefined); return; }
    const delay = RECONNECT_DELAYS_MS[this.reconnectAttempt]; const attempt = this.reconnectAttempt + 1; this.reconnectAttempt = attempt;
    this.emit({ type: 'reconnecting', sessionId: this.resumeSessionId, role: 'initiator', attempt });
    this.reconnectTimer = setTimeout(() => {
      this.reconnectTimer = null;
      void this.enqueue(async () => {
        const context = this.context; const endpoint = this.outboundEndpoint; const sessionId = this.resumeSessionId;
        if (!context || !endpoint || !sessionId || this.resumeRole !== 'initiator' || this.active) return;
        try { await this.openInitiator(endpoint, sessionId, context, false); } catch { this.scheduleInitiatorReconnect(); }
      }).catch(() => undefined);
    }, delay);
  }
  ...
  private armResponderReconnectWindow(): void {
    ...
    this.responderReconnectTimer = setTimeout(() => { this.responderReconnectTimer = null; void this.enqueue(() => this.failReconnect()).catch(() => undefined); }, RESPONDER_RECONNECT_WINDOW_MS);
  }
  private async failReconnect(): Promise<void> {
    const sessionId = this.resumeSessionId; if (!sessionId) return;
    this.clearReconnectState(); this.emit({ type: 'error', code: 'transport_failed' }); this.emit({ type: 'closed', sessionId });
  }

  private async handleFrame(frame: string): Promise<void> {
    const active = this.active; const context = this.context; if (!active || !context) return;
    if (!active.sessionKeyHex) {
      ...
      if (this.resumeSessionId) {
        await this.transport.close(active.connectionId).catch(() => undefined); if (this.active === active) this.active = null;
        if (active.role === 'initiator') this.scheduleInitiatorReconnect(); return;
      }
      this.emit({ type: 'error', code: 'auth_failed' }); await this.closeActiveNow(false); return;
    }
    try {
      ...
      if (!validation?.ok) { this.emit({ type: 'error', code: 'invalid_message' }); await this.closeActiveNow(false); return; }
      this.emit({ type: 'message', message });
    } catch { this.emit({ type: 'error', code: 'invalid_message' }); await this.closeActiveNow(false); }
  }
  ...
  private async acceptHello1(active: ActiveConnection, context: ControlTrustContext, hello: Hello1Frame): Promise<void> {
    ...
    active.sessionId = hello.sessionId; active.initiatorNonce = hello.nonce; active.responderNonce = responderNonce; active.sessionKeyHex = key;
    active.validator = new MessageValidator(context.partnerDeviceId, hello.sessionId, this.nowMs); active.nextSendSequence = 1; this.clearHandshake(active);
    await this.transport.send(active.connectionId, encodeHandshakeFrame(response));
    if (this.resumeSessionId) { this.clearReconnectState(); this.emit({ type: 'reconnected', sessionId: hello.sessionId, role: 'responder' }); }
    else this.emit({ type: 'authenticated', sessionId: hello.sessionId, role: 'responder' });
  }
  ...
  private async acceptHello2(active: ActiveConnection, context: ControlTrustContext, hello: Hello2Frame): Promise<void> {
    ...
    active.responderNonce = hello.nonce; active.sessionKeyHex = key; active.validator = new MessageValidator(context.partnerDeviceId, active.sessionId, this.nowMs); active.nextSendSequence = 1; this.clearHandshake(active);
    active.authResolve?.(active.sessionId); active.authResolve = null; active.authReject = null;
    if (this.resumeSessionId) { const sessionId = active.sessionId; this.clearReconnectState(); this.emit({ type: 'reconnected', sessionId, role: 'initiator' }); }
    else this.emit({ type: 'authenticated', sessionId: active.sessionId, role: 'initiator' });
  }
  ...
  private async ensureListeningNow(expectedHost?: string): Promise<ControlListenerEndpoint> {
    ...
    if (this.listenerEndpoint) {
      ...
      const stale = this.listenerEndpoint; this.listenerEndpoint = null; await this.transport.stopListener(stale.listenerId).catch(() => undefined);
      this.emit({ type: 'listener_changed' }); for (const listener of this.listenerChangeListeners) listener();
    }
    this.listenerEndpoint = await this.transport.startListener(); return this.listenerEndpoint;
  }

  private async deactivateNow(): Promise<void> {
    await this.closeActiveNow(true); const listener = this.listenerEndpoint; this.listenerEndpoint = null;
    if (listener) await this.transport.stopListener(listener.listenerId).catch(() => undefined);
    await this.transport.stopTrustedPresence?.().catch(() => undefined); this.context = null; this.outboundEndpoint = null;
  }
  private async closeActiveNow(clearReconnect: boolean): Promise<void> {
    if (clearReconnect) this.clearReconnectState(); const active = this.active; if (!active) return;
    this.active = null; this.clearHandshake(active); active.authReject?.(new Error('Control session closed.')); active.authReject = null; active.authResolve = null;
    try { await this.transport.close(active.connectionId); } catch { /* best effort */ }
  }
  private clearReconnectState(): void {
    if (this.reconnectTimer) clearTimeout(this.reconnectTimer); if (this.responderReconnectTimer) clearTimeout(this.responderReconnectTimer);
    this.reconnectTimer = null; this.responderReconnectTimer = null; this.resumeSessionId = null; this.resumeRole = null; this.reconnectAttempt = 0;
  }
