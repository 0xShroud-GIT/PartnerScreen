...
function parseEvent(value: unknown): PartnerDiscoveryEvent | null {
  ...
  if (item.type === 'service_lost') {
    ...
    return typeof item.serviceName === 'string' && SERVICE_NAME_RE.test(item.serviceName)
      ? { type: 'service_lost', serviceName: item.serviceName }
      : null;
  }
  if (item.type === 'error') {
    if (Object.keys(item).some((key) => !['type', 'code'].includes(key))) return null;
    return typeof item.code === 'string' && ERROR_CODE_RE.test(item.code)
      ? { type: 'error', code: item.code }
      : null;
  }
  return null;
}
...
export class ExpoPartnerDiscovery implements PartnerDiscovery {
  async prepareAdvertisement(): Promise<DiscoveryAdvertisementPreparation> {
    try {
      ...
      if (
        !value ||
        typeof value.advertisementId !== 'string' || value.advertisementId.length > 128 ||
  ...
  async start(advertisementId: string, peerHint: string, proof: string): Promise<DiscoveryRegistration> {
    try {
      ...
      if (!value || typeof value.serviceName !== 'string' || !SERVICE_NAME_RE.test(value.serviceName)) {
  ...
  async probe(host: string, port: number): Promise<void> {
    try {
      await getNativeModule().probe(host, port);
    } catch {
      throw new Error('The discovered trusted endpoint is not reachable on this Wi-Fi.');
    }
  }
